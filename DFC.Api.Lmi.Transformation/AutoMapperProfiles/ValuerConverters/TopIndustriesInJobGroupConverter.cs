...
namespace DFC.Api.Lmi.Transformation.AutoMapperProfiles.ValuerConverters
{
    ...
    public class TopIndustriesInJobGroupConverter : IValueConverter<IList<IBaseContentItemModel>?, List<BreakdownModel>?>
    {
            ...
            foreach (var item in sourceMember)
            {
                switch (item.ContentType)
                {
                    case nameof(LmiSocBreakdown):
                        if (item is LmiSocBreakdown lmiSocBreakdown && lmiSocBreakdown.Measure != null && lmiSocBreakdown.Measure.Equals(Constants.MeasureForIndustry))
                        {
